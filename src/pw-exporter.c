...
prom_gauge_t *xrun_gauge;
prom_gauge_t *state_gauge;
prom_gauge_t *quantum_gauge;
prom_gauge_t *rate_gauge;
prom_gauge_t *wait_gauge;
prom_gauge_t *busy_gauge;
prom_gauge_t *wait_quantum_gauge;
prom_gauge_t *busy_quantum_gauge;

void metrics_init(void) {
    xrun_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("xrun_gauge", "Number of XRUNs", 1, (const char *[]) {"name"}));
    state_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("state_gauge", "Current state", 1, (const char *[]) {"name"}));
    quantum_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("quantum_gauge", "Current quantum value", 1, (const char *[]) {"name"}));
    rate_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("rate_gauge", "Current bitrate value", 1, (const char *[]) {"name"}));
    wait_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("wait_gauge", "Current wait time", 2, (const char *[]) {"name", "active"}));
    busy_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("busy_gauge", "Current busy time", 2, (const char *[]) {"name", "active"}));
    wait_quantum_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("wait_quantum_gauge", "Current wait quantum time", 2, (const char *[]) {"name", "active"}));
    busy_quantum_gauge = prom_collector_registry_must_register_metric(
            prom_gauge_new("busy_quantum_gauge", "Current busy quantum time", 2, (const char *[]) {"name", "active"}));
}
...
static float get_quantum(bool active, size_t len, uint64_t val, float quantum) {
    if (val == (uint64_t) - 1 || !active) {
        ...
        return quantum == 0.0f ? 0.0f : frac / quantum;
    }
}

// I need to document this somewhere because we can only return numeric metrics
...
static void update_metrics(struct data *d, struct driver *i, struct node *n, int y) {
    char buf1[64];
    char buf2[64];
    char buf3[64];
    char buf4[64];
    ...
    const char *active_str = (active == true) ? "true" : "false";

    prom_gauge_add(state_gauge, n->state, (const char *[]) {n->name});
    prom_gauge_add(quantum_gauge, frac.num, (const char *[]) {n->name});
    prom_gauge_add(rate_gauge, frac.denom, (const char *[]) {n->name});
    prom_gauge_add(wait_gauge, waiting, (const char *[]) {n->name, active_str});
    prom_gauge_add(busy_gauge, busy, (const char *[]) {n->name, active_str});
    prom_gauge_add(wait_quantum_gauge, get_quantum(active, 64, waiting, quantum),
                   (const char *[]) {n->name, active_str});
    prom_gauge_add(busy_quantum_gauge, get_quantum(active, 64, busy, quantum), (const char *[]) {n->name, active_str});
    n->measurement.xrun_count == XRUN_INVALID ?
    prom_gauge_add(xrun_gauge, i->xrun_count, (const char *[]) {n->name}) :
    prom_gauge_add(xrun_gauge, n->measurement.xrun_count, (const char *[]) {n->name});
}
...
void *run_prom_httpd() {
    ...
    if (( pTmp =getenv("METRICS_PORT") ) != NULL ) {
        port = atoi(pTmp);
    } else {
        port = 8000;
    }
...
int main(int argc, char *argv[]) {
    ...
    pthread_t prom_httpd_thread;

    pthread_create(&prom_httpd_thread, NULL, *run_prom_httpd, NULL);
    ...
    pthread_kill(prom_httpd_thread, SIGINT);
